...
function getScrapperOptionsByUrl(url: string, title: string) {
  ...
  if (
    ...
    url.includes("tiktok.com/search/video")
  ) {
    return {
      header: "TikTok Search Results",
    ...
    return {
      ...
      elementParser: [
        ...
        },
        {
          title: "Description",
          query:
            "td:nth-child(3) > div > div:nth-child(2) > div > div:nth-child(2) > a",
          type: "text",
        },
  ...
  if (
    url.includes("idealista.pt") &&
    (url.includes("/comprar-") || url.includes("/arrendar-"))
  ) {
    return {
  ...
  if (
    ...
    (url.includes("/for_") || url.includes("?search"))
  ) {
    return {
      header: title,
      ...
      elementParser: [
        ...
        },
        {
          title: "Area",
          query:
            'ul[class*="StyledPropertyCardHomeDetailsList"] li:nth-child(3)',
          type: "text",
        },
        {
          title: "Zillow link",
  ...
  await chrome.scripting.executeScript({
    ...
    func: (tsv) => {
      window.localStorage.setItem(
        "rows_x",
        JSON.stringify({ source: "%ROWS_X%", data: tsv }),
      );
    },
  });
}
