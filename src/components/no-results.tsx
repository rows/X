...
const NoResults = () => {
    const redirectToFeedback = async () => {
        ...
        const message = "Hi Rows team,\n\n"+
            `I'd like to add "${tab.url}" to be compatible with RowsX.\n\nBest,\n\n--`;

        window.open(`mailto:enterprise@rows.com?subject=Request RowsX to be compatible with ${url.origin}&body=${encodeURIComponent(message)}`);
    }
