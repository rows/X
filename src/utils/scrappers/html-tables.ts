export async function scrapHTMLTables() {
  function removeEmptyColumns(arr: any[]) {
    ...
    return arr.map((column) =>
      column.filter((_: any, index: number) => emptyColumns[index]),
    );
  }
  ...
  function toArray(table: any) {
    ...
    return removeEmptyColumns(
      data.map((row: any) => row.map((col: any) => col.value)),
    );
  }
  ...
  const tables = tableElements.map((tableElement: any) => {
    ...
    return Array.from(tableElement.querySelectorAll(rowSelector)).map(
      (tr: any) => {
        if (!tr) {
          return [];
        }
  ...
  return tables
    ...
    .filter(
      (table) =>
        !table.table.every((row: any) => row.every((col: any) => col === "")),
    );
}
